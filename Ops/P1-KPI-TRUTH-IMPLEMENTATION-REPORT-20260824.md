...
| 8 | `ca28cf5` | إصلاح B-1: مفتاح أسبوع اللوحة يتبع دورة الرياض + 7 اختبارات انحدار | 3 |
...
$ git rev-parse HEAD
ca28cf59f35d6cf0bca688f868e2ecfb2465db1d

$ git status --short
(فارغة — لا تعديلات غير ملتزَمة ولا ملفّات غير متعقَّبة)
```

سلسلة الالتزامات الثمانية من الـBaseline:
`736b5c5` → `a06ac35` → `2392ddc` → `c86fa81` → `7269fbf` → `3501b85` → `7bf52ae` → `f1abb39` → `ca28cf5`
