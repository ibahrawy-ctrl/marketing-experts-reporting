...
> **حالة نافذة (مُحدَّثة 7 سبتمبر 2026):** الحاجبان اللذان جعلا حكم هذا التقرير `BLOCKED` يوم 6 سبتمبر (`EMPLOYEE_LOGIN` و`TEAM_LEADER_LOGIN`) **أُغلِقا بالبرهان المباشر على الإنتاج**، والحكم النافذ الآن `R22B_REPORTING_PRODUCTION_RELEASE = PASS` (§9). التقرير الحاكم للجولة المُغلِقة: **`PRODUCTION-AUTHENTICATED-CYCLE-CLOSURE.md`** وأدلّته في `evidence/production-authenticated-cycle/`. أقسام هذا التقرير التي تصف نافذة 6 سبتمبر تُقرأ سجلًّا تاريخيًّا، وقد وُسِمت داخليًّا حيث تغيّرت.
...
### 5-ج) ما لم يُنفَّذ عند تحرير هذه النسخة — ثمّ أُغلِق لاحقًا في 7 سبتمبر
> **تحديث حاكم (7 سبتمبر 2026):** الحاجبان أدناه **أُغلِقا فعليًّا** بجولة مصادَق عليها كتب المالك بنفسه بيانات دخولها لحسابين قائمين مترابطين تشغيليًّا (موظّف ↔ قائدة فريقه الفعليّة). النصّ الأصليّ يبقى أدناه سجلًّا تاريخيًّا بلا تعديل، والحالة النافذة هي حالة §9 بعد التحديث. البرهان الكامل: **`PRODUCTION-AUTHENTICATED-CYCLE-CLOSURE.md`** وأدلّته في `evidence/production-authenticated-cycle/`.
>
> ```
> EMPLOYEE_LOGIN          = PASS            (كان NOT_RUN_ROLE_NOT_SUPPLIED)
> TEAM_LEADER_LOGIN       = PASS            (كان NOT_RUN_ROLE_NOT_SUPPLIED)
> PRODUCTION_WRITE_CANARY = DONE (canary واحد: 96613974-ed0d-41bc-8cae-78c4511a4004 · 2026-W37)
> WEBKIT_AUTHENTICATED    = NOT_RUN_WOULD_REQUIRE_TOKEN_EXTRACTION   (باقٍ — ممنوع صراحةً)
> ```

**النصّ الأصليّ (6 سبتمبر) — يُقرأ تاريخيًّا لا تشغيليًّا:**
```
...
```
**السبب الدقيق حينها:** التصريح يوجب استعمال حسابات قائمة فقط وأن يكتب المالك بيانات الدخول بنفسه، ويمنع إنشاء حساب أو تصفير كلمة مرور. الجلسة الوحيدة المتاحة دور **CEO**؛ ودورة القرار تحتاج **موظّفًا** يُنشئ التقرير و**قائد فريق** يراجعه — والـCEO في قمّة السلسلة فلا مراجِع فوقه. لذلك لم تُقس حينها على الإنتاج: دورة حياة التقرير (إدخال ← حفظ مسوّدة ← إعادة تحميل ← إرسال)، ودورة القائد (طابور ← تعليق متعدّد الأسطر ← إرجاع ← إعادة إرسال ← اعتماد). **لم يُختلق دليل عليها.** أمّا WebKit المصادَق عليه فيستلزم نقل توكن/كوكي بين المحرّكين وهو **ممنوع صراحةً**، وWebKit مُثبَت مجهولًا 2/2.

وقد كانت هذه المسارات **مقيسة ومغلقة على RC** الذي يشغّل **المصدر نفسه** (`c5e0202`) بقطعة واجهة **مطابقة بايتيًّا** وبلا فارق تهيئة عدا عنوان الـAPI بالتصميم — ثمّ أصبحت **مقيسة مباشرةً على الإنتاج** في 7 سبتمبر.
...
| VIS-05 الأسطح الحاكمة (طابور · أرشيف · قوالب · تقويم · Project 360) | **PASS** (كان «PASS جزئيّ — عرضًا فقط» في 6 سبتمبر) | الأسطح تُحمَّل وتُصيَّر سليمة؛ و**أفعال القرار جُرِّبت فعليًّا على الإنتاج** في 7 سبتمبر: حفظ مسوّدة · إرسال · إرجاع بتعليق ثلاثيّ الأسطر · إعادة إرسال · اعتماد ⟸ `PRODUCTION-AUTHENTICATED-CYCLE-CLOSURE.md` |

```
VIS_01 = PASS   VIS_02 = PASS   VIS_03 = PASS   VIS_04 = PASS
VIS_05 = PASS  (7 سبتمبر — أفعال القرار مُجرَّبة على الإنتاج بجلستَي موظّف وقائدة فريق)
...
VIS_03/VIS_04 (الجولة المصادَق عليها 7 سبتمبر) = PASS 20/20 قياسًا
```
...
**لم يُنشأ أيّ canary على الإنتاج في نافذة 6 سبتمبر ⟹ لا شيء يُنظَّف فيها.** لم يُحذف أيّ شيء حذفًا صلبًا، ولم يُمسّ أيّ مورد آخر.
**تحديث 7 سبتمبر:** أُنشئ **canary واحد** موسوم (`96613974-ed0d-41bc-8cae-78c4511a4004`) ضمن الجولة المصادَق عليها، ومصيره `RETAINED_WITH_VALID_AUDIT_REASON` — التفصيل والضوابط السالبة في `PRODUCTION-AUTHENTICATED-CYCLE-CLOSURE.md` §7 و§8.

## 8) قيود معروفة ومسجَّلة (لا تُطوى)

1. ~~**دورة الكتابة على الإنتاج لم تُجرَّب**~~ — **قيد مرفوع في 7 سبتمبر 2026.** نُفِّذت الدورة كاملة على الإنتاج بجلسة **موظّف** وجلسة **قائدة فريقه الفعليّة** كتب المالك بيانات دخولهما بنفسه: حفظ مسوّدة · إرسال · طابور القائدة · إرجاع بتعليق ثلاثيّ الأسطر · تعديل · إعادة إرسال · اعتماد · ظهور مغلق للطرفين وفي Project 360. البرهان: `PRODUCTION-AUTHENTICATED-CYCLE-CLOSURE.md`.
...
VIS_01 = PASS  VIS_02 = PASS  VIS_03 = PASS  VIS_04 = PASS  VIS_05 = PASS
EMPLOYEE_LOGIN                     = PASS      (7 سبتمبر · حساب قائم كتبه المالك)
TEAM_LEADER_LOGIN                  = PASS      (7 سبتمبر · قائدة الفريق الفعليّة للموظّف)
REVIEW_RELATIONSHIP_VERIFIED       = PASS      (الموظّف ظهر في طابور القائدة فعليًّا)
PRODUCTION_WRITE_CANARY            = DONE      (canary واحد · 2026-W37 · قالب v7)
...
> **حالة هذا القسم:** الحكم أعلاه هو **الحكم النافذ بعد تحديث 7 سبتمبر 2026**. كان في نسخة 6 سبتمبر `R22B_REPORTING_PRODUCTION_RELEASE = BLOCKED` بحاجبَين هما `EMPLOYEE_LOGIN` و`TEAM_LEADER_LOGIN = NOT_RUN_ROLE_NOT_SUPPLIED` — وقد أُغلِقا بالبرهان المباشر على الإنتاج، لا بالتنازل. **التقرير الحاكم للجولة المُغلِقة: `PRODUCTION-AUTHENTICATED-CYCLE-CLOSURE.md`.**

**قراءة الحكم بلا لبس:** لم يُكتشَف أيّ عيب سلوكيّ على الإنتاج في أيّ مرحلة: النشر سليم، والقطعة مطابقة، والأسطح تُصيَّر صحيحة، وحارس الأسطر المتعدّدة يعمل على بيانات حقيقيّة عند الإدخال وعند المراجعة وبعد الاعتماد، والرفض لا يفشي الوجود. الحاجب السابق كان **نقص دليل** لا عطبًا، وقد ارتفع بجولة قرار كاملة على الإنتاج بحسابين قائمين مترابطين تشغيليًّا، بـcanary واحد فقط وصفر أثر على أيّ سجلّ آخر (ضوابط سالبة ببصمات `md5` قبل/بعد).

**القيد الوحيد الباقي (غير حاجب):** `WEBKIT_AUTHENTICATED = NOT_RUN` لأنّه يستلزم نقل توكن بين المحرّكين وهو ممنوع صراحةً؛ وWebKit مُثبَت مجهولًا 2/2 وعلى RC 10/10.
