...
public static class TemplateSeeder
{
    ...
    private static async Task SeedKpiTemplatesAsync(AppDbContext db, Guid ownerId)
    {
        ...
        {
            // R6.3/§3 — لا يُبذر ولا يُنشَر قالب تقييم ربعيّ جديد. المسار الربعيّ متقاعد: إنشاء
            // التقييم مقفل منذ R6/§5.4 وكلّ كتابة عليه مقفلة منذ R6.1–R6.3، فبذر قالب ربعيّ منشور
            // كان يُنتج مصدرًا حيًّا لمسار لا يمكن استعماله (استحقاقات لا تُوفَّى وقوالب لا تُملأ).
            // الحارس **تخطٍّ لا حذف**: القوالب الربعيّة التاريخيّة القائمة في قاعدة البيانات تبقى كما
            // هي للقراءة والأرشيف (WS-2)، ولا هجرة ولا حذف. والتعريفان يبقيان في `KpiDefs` سجلًّا
            // تاريخيًّا مقروءًا لا مصدرَ بذر. الباذر يبقى idempotent: التخطّي ثابت في كلّ تشغيل.
            if (def.Cadence == KpiCadence.Quarterly) continue;
