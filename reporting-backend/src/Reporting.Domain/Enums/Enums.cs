...
/// <summary>
/// الحالة الظاهرة لصحّة المشروع (P360-WF-R2 §7) — **أربع حالات ظاهرة** + حالة «لم تُقيَّم».
///
/// <para>
/// **توافق القيم الرقميّة مع البيانات القائمة مقصود وحاسم:** <c>0</c> و<c>1</c> و<c>2</c> تحتفظ
/// بمعناها الدلاليّ السابق (أخضر · أصفر=في خطر · أحمر=متأخّر) فلا يحتاج أيّ صفّ مخزَّن إعادة
/// كتابة، وإعادة التسمية وحدها لا تلمس القاعدة. الجديد إضافةً بحتة: <see cref="Blocked"/>
/// و<see cref="NotEvaluated"/>.
/// </para>
///
/// <para>
/// <see cref="NotEvaluated"/> تُغلِق GAP-05: مشروع لم تُحتسَب صحّته قطّ كان يُخزَّن <c>Green</c>
/// لأنّ صفر هو الافتراضيّ — أي أنّ المشروع المهمَل كان الأكثر ظهورًا بالأخضر. الهجرة المصاحبة
/// تنقل كلّ صفّ بـ<c>HealthComputedAtUtc IS NULL</c> إلى هذه القيمة.
/// </para>
/// </summary>
public enum ProjectHealthStatus
{
    /// <summary>لا حاجب ولا تأخّر حرج، والمؤشّرات والتقدّم ضمن الحدود المقبولة.</summary>
    Green = 0,

    /// <summary>مخاطر مرتفعة مفتوحة أو اتّجاه نحو التأخّر دون تحقّق شرط <see cref="Delayed"/>.</summary>
    AtRisk = 1,

    /// <summary>مخرَج أساسيّ تجاوز استحقاقه دون إتمام، أو التقدّم دون المتوقَّع زمنيًّا بفارق جوهريّ.</summary>
    Delayed = 2,

    /// <summary>حاجب مفتوح مثبت — يتقدّم على كلّ حساب آخر مهما بلغت المؤشّرات.</summary>
    Blocked = 3,

    /// <summary>لم تُحتسَب الصحّة بعد. **ليست حكمًا على المشروع** بل غياب حكم.</summary>
    NotEvaluated = 4
}

/// <summary>
/// وضع احتساب تقدّم المشروع (P360-WF-R2 §6.1). يُخزَّن مع النسبة كي يعرف القارئ **كيف** حُسبت،
/// فرقم بلا وضعٍ يُقرأ ثقةً في غير موضعها.
/// </summary>
public enum ProjectProgressMode
{
    /// <summary>لم يُحتسَب بعد — لا يُقرأ رقم التقدّم على أنّه صفر حقيقيّ.</summary>
    NotCalculated = 0,

    /// <summary>متوسّط موزون بأوزان المخرَجات: كلّ المخرَجات النشطة لها وزن صحيح ومجموعها 100.</summary>
    Weighted = 1,

    /// <summary>
    /// الأوزان غير مضبوطة أو ناقصة ⟹ توزيع متساوٍ. **يستوجب تنبيهًا ظاهرًا** بأنّ الأوزان افتراضيّة.
    /// </summary>
    EqualWeightFallback = 2,

    /// <summary>لا مخرَجات نشطة ⟹ النسبة 0 **بتفسير** لا برقم مضلّل.</summary>
    NoDeliverables = 3
}

/// <summary>
/// حالة مقترح تحديث تنفيذ (P360-WF-R2 §10 — جسر التنفيذ الهجين).
///
/// <para>
/// **لماذا مقترح لا كتابة مباشرة؟** عضو الفريق يعرف ما أنجزه، لكنّه ليس من يقرّر أنّ المخرَج
/// التعاقديّ تقدّم — تلك مسؤوليّة قائد الفريق أمام العميل. الكتابة المباشرة كانت ستجعل تقدّم
/// المشروع مجموعَ ادّعاءات غير مراجَعة؛ والاكتفاء بالعرض كان سيُبقي الفجوة التي فُتحت التذكرة
/// لإغلاقها. المقترح يفصل **الإخبار** عن **الإقرار** بلا أن يُلغي أيّهما.
/// </para>
/// </summary>
public enum ExecutionProposalStatus
{
    /// <summary>مُقدَّم وبانتظار مراجعة قائد الفريق. لا أثر له على أيّ رقم.</summary>
    Pending = 0,

    /// <summary>قُبِل وطُبِّق على المخرَج — والتطبيق يقع **مرّة واحدة** بحكم هذه الحالة نفسها.</summary>
    Accepted = 1,

    /// <summary>رُفِض بسبب معلن. يبقى محفوظًا: الرفض جزء من السجلّ لا حذفٌ منه.</summary>
    Rejected = 2
}
