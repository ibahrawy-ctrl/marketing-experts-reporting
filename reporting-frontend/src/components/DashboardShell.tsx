// الهيكل العام (NAVIGATION-REGRESSION-HOTFIX-R1) — شريط جانبي بوحداتٍ رئيسية محدودة + شريط تبويبات
// أفقي للصفحات الفرعية داخل الوحدة النشطة. تنظيم بصريّ فقط: يقرأ نموذج التنقّل من navConfig
// (لا مسار جديد/محذوف، لا توسيع/تضييق صلاحية). الحماية الفعلية مفروضة عبر ProtectedRoute + سياسات الخادم.
...
// مفتاح طيّ الشريط الجانبي إلى شريط أيقونات على سطح المكتب (مفتاح v2، لكل متصفّح).
...
export function DashboardShell({ children }: { children: ReactNode }) {
  ...
  const ctx = useMemo<NavCtx>(() => ({ hasAnyRole, isSalesRep }), [hasAnyRole, isSalesRep]);
