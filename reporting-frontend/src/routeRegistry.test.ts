...
describe('سجلّ المسارات', () => {
  ...
  // P123-R2 — الإبراز يُقاس على النمط **الذي طابَق فعلًا**، لا على طول وجهةٍ لم تُطابِق.
  // مسار `/app/employees` يملكه الدليل تمامًا، بينما تعلن لوحة الأداء البادئة نفسها لالتقاط
  // `/app/employees/{id}/kpi`. القاعدة القديمة (أطول وجهة تفوز) كانت تمنح الجذرَ للوحة الأداء
  // لمجرّد أنّ وجهتها أطول نصًّا — فيسرق عنصرٌ إبرازَ مسارٍ لا يملكه.
  it('صاحب المسار التامّ أولى بجذره، ومن أعلن البادئة يملك ما تحتها', () => {
    expect(resolveActive('/app/employees')?.item.id).toBe('people.directory');
    expect(resolveActive('/app/employees/11111111-1111-1111-1111-111111111111/kpi')?.item.id).toBe(
